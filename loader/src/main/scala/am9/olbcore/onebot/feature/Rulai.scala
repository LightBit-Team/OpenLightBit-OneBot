...
object Rulai {
  private val ruText: Array[String] =
    """中国人认为宇宙万法的那个源头
      |它是什么
      |它是如如
      |对吧
  ...
  def sendRu(uid: Long): Unit = {
    ...
    Main.oneBot.sendFriend(uid, ruText.apply(index.get(uid)).substring(0, ruText.apply(index.get(uid)).length - 1))
    index.put(uid, index.get(uid) + 1)
    if (index.get(uid) >= ruText.length - 1) {
      index.put(uid, 0)
    }
  }
}
